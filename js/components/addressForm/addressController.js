(function(){
 ...
 $scope.photoChanged = function(files) {
      ...
      reader.onload = function(e) {
        $scope.addressModel.user.EmployeeIDProof = e.target.result;
        if (!$scope.$$phase) {
          $scope.$apply();
        }
...
function getUserData(usr){
   ...
   /*var femp = document.getElementById('empPhoto').files[0],
          rphoto = new FileReader();
      rphoto.onloadend = function(ep){
        //$scope.data = e.target.result;
        $scope.addressModel.user.EmployeePhoto = ep.target.result;
        console.log("empphoto",$scope.addressModel.user.EmployeePhoto)
      }
      rphoto.readAsBinaryString(femp);
      // end

var f = document.getElementById('empIdproof').files[0],
          rid = new FileReader();
      rid.onloadend = function(e){
        //$scope.data = e.target.result;
        $scope.addressModel.user.EmployeeIDProof = e.target.result;
        console.log("empIdproof",$scope.data)
      }
      rid.readAsBinaryString(f);
  ...
  "EmployeePhoto" : usr.EmployeePhoto,
	"EmployeeIDProof" : usr.EmployeeIDProof
	};
